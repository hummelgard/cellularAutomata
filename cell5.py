...
ALGORITHM = 'ink'								# What type of algorithm to use XOR4, XOR8, INK, etc.
...
# Load a matrix from a text file, this is the same format as the ones being saved during iterations.
def LoadTxtMatrix(fileName):
...
matrix = np.array(np.floor(np.random.sample(size=(SIZE,SIZE))+RATIO/100.0),dtype=bool)
...
for i in range(RUNS):
	...
	matrix=np.roll(matrix,1, axis=0)^np.roll(matrix,-1, axis=0)^np.roll(matrix,1, axis=1)^np.roll(matrix,-1, axis=1)
	for y in range(SIZE):
		for x in range(SIZE):
